...
namespace gl
{
    ...
    void resetFosucPlanet(std::vector<Planet>& planets, Planet* planet1 = new Planet());
    void resetStatusButton(std::vector<Button>& buttons);
    void resetMenuPlanets(std::vector<Planet>& planets);
    void logicPlanet(std::vector<Planet>& planets, double& G, double& scalePhy, bool gravity);
